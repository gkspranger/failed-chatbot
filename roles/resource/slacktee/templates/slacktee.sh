...
# ----------
webhook_url="{{ bot_clients_secrets[ ec2_tags.tags.client ].slack_webhook_url }}"       # Incoming Webhooks integration URL
upload_token=""      # The user's API authentication token, only used for file uploads
channel="garage"     # Default channel to post messages. '#' is prepended, if it doesn't start with '#' or '@'.
tmp_dir="/tmp"       # Temporary file is created in this directory.
username="{{ ec2_tags.tags.bot_name }}"  # Default username to post messages.
icon="computer"         # Default emoji to post messages. Don't wrap it with ':'. See http://www.emoji-cheat-sheet.com; can be a url too.
attachment=""        # Default color of the attachments. If an empty string is specified, the attachments are not used.

# gks: to avoid templating errors
{% raw %}
